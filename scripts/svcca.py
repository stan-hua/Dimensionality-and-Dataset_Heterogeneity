...
def get_train_test(paths: list, dataset_used: str,
                   pca_option=False, start=0, end=4, num_dims: int = 20,
                   cpv=False):
    """
    Parameters
    ----------
...
def process_train_test(inputs: Inputs, pca_option: bool,
    ...
    if pca_option:
        ...
        pca_model = MyPCA(inputs.chosen_features)
...
def get_sample_sizes(paths: list, dataset_used: str,
    ...
    train_dfs, test_dfs = get_train_test(paths, dataset_used,
                                         pca_option=True,
                                         start=start,
                                         end=end, num_dims=1)
...
def compare_cv_svcca_all(start: int = 0, end: int=16, num_dims: int = 20):
    global paths, dataset_num
    sample_sizes = get_sample_sizes(paths, dataset_num,
                                    start=start,end=end, unique=True)
...
if __name__ == "__main__":
    ...
    plt.title(f"{dataset_used} | CPV >= 0.99 Principal Components")
